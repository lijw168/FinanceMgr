...
func (dao *LoginInfoDao) Get(ctx context.Context, do DbOperator, optID int) (*model.LoginInfo, error) {
	...
	gLogger.DebugContext(ctx, "[LoginInfo/db/Get] [sql: %s ,values: %d]", strSql, optID)
	var optInfo = &model.LoginInfo{}
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[LoginInfo/db/Get] [SqlElapsed: %v]", time.Since(start))
	}()
	...
	case sql.ErrNoRows:
		return nil, err
	default:
		gLogger.ErrorContext(ctx, "[LoginInfo/db/Get] [scanloginInfo: %s]", err.Error())
		return nil, err
	}
}
...
func (dao *LoginInfoDao) CountByFilter(ctx context.Context, do DbOperator, filter map[string]interface{}) (int64, error) {
	...
	gLogger.InfoContext(ctx, "[LoginInfo/db/CountByFilter] [SqlElapsed: %v]", time.Since(start))
	return c, err
}
...
func (dao *LoginInfoDao) Create(ctx context.Context, do DbOperator, st *model.LoginInfo) error {
	...
	gLogger.DebugContext(ctx, "[LoginInfo/db/Create] [sql: %s, values: %v]", strSql, values)
	start := time.Now()
	_, err := do.ExecContext(ctx, strSql, values...)
	gLogger.InfoContext(ctx, "[LoginInfo/db/Create] [SqlElapsed: %v]", time.Since(start))
	if err != nil {
		gLogger.ErrorContext(ctx, "[LoginInfo/db/Create] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
...
func (dao *LoginInfoDao) Delete(ctx context.Context, do DbOperator, optID int) error {
	...
	gLogger.DebugContext(ctx, "[LoginInfo/db/Delete] [sql: %s, id: %s]", strSql, optID)
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[LoginInfo/db/Delete] [SqlElapsed: %v]", time.Since(start))
	}()
	if _, err := do.ExecContext(ctx, strSql, optID); err != nil {
		gLogger.ErrorContext(ctx, "[LoginInfo/db/Delete] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
...
func (dao *LoginInfoDao) List(ctx context.Context, do DbOperator, filter map[string]interface{},
	...
	gLogger.DebugContext(ctx, "[LoginInfo/db/List] sql %s with values %v", strSql, values)
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[LoginInfo/db/List] [SqlElapsed: %v]", time.Since(start))
	}()
	result, err := do.QueryContext(ctx, strSql, values...)
	if err != nil {
		gLogger.ErrorContext(ctx, "[LoginInfo/db/List] [do.Query: %s]", err.Error())
	...
	for result.Next() {
		...
		if err != nil {
			gLogger.ErrorContext(ctx, "[LoginInfo/db/List] [ScanSnapshot: %s]", err.Error())
			return LoginInfoSlice, err
		}
...
func (dao *LoginInfoDao) Update(ctx context.Context, do DbOperator, filter map[string]interface{},
	...
	gLogger.DebugContext(ctx, "[LoginInfo/db/Update] sql %s with values %v", strSql, values)
	start := time.Now()
	_, err := do.ExecContext(ctx, strSql, values...)
	gLogger.InfoContext(ctx, "[LoginInfo/db/Update] [SqlElapsed: %v]", time.Since(start))
	if err != nil {
		gLogger.ErrorContext(ctx, "[LoginInfo/db/Update] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
