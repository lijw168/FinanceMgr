...
func (dao *YearBalanceDao) GetYearBalance(ctx context.Context, do DbOperator,
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/GetYearBalance] [sql: %s ,comId:%d,year:%d,subId:%d]",
		strSql, *params.CompanyID, *params.Year, *params.SubjectID)
	yearBal := &model.YearBalance{}
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/GetYearBalance] [SqlElapsed: %v]", time.Since(start))
	}()
	...
	case nil:
		return yearBal, nil
	case sql.ErrNoRows:
		gLogger.ErrorContext(ctx, "[yearBalance/db/GetYearBalance] [scanYearBalanceTask: %s]", err.Error())
		return nil, err
	default:
		gLogger.ErrorContext(ctx, "[yearBalance/db/GetYearBalance] [Scan: %s]", err.Error())
		return nil, err
	}
}
...
func (dao *YearBalanceDao) GetAccSubYearBalValue(ctx context.Context, do DbOperator,
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/GetAccSubYearBalValue] [sql: %s ,comId:%d,year:%d,subId:%d]",
		strSql, *params.CompanyID, *params.Year, *params.SubjectID)
	var dBalanceValue float64
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/GetAccSubYearBalValue] [SqlElapsed: %v]", time.Since(start))
	}()
	...
	case sql.ErrNoRows:
		gLogger.ErrorContext(ctx, "[yearBalance/db/GetAccSubYearBalValue] [scanAccSubTask: %s]", err.Error())
		return 0, err
	default:
		gLogger.ErrorContext(ctx, "[yearBalance/db/GetAccSubYearBalValue] [scanAccSubTask: %s]", err.Error())
		return 0, err
...
func (dao *YearBalanceDao) CreateYearBalance(ctx context.Context, do DbOperator, st *model.YearBalance) error {
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/CreateYearBalance] [sql: %s, values: %v]", strSql, values)
	start := time.Now()
	_, err := do.ExecContext(ctx, strSql, values...)
	gLogger.InfoContext(ctx, "[yearBalance/db/CreateYearBalance] [SqlElapsed: %v]", time.Since(start))
	if err != nil {
		gLogger.ErrorContext(ctx, "[yearBalance/db/CreateYearBalance] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
...
func (dao *YearBalanceDao) DeleteYearBalance(ctx context.Context, do DbOperator, params *model.BasicYearBalanceParams) error {
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/DeleteYearBalance] [sql: %s, comId:%d,year:%d,subId:%d]",
		strSql, *params.CompanyID, *params.Year, *params.SubjectID)
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/DeleteYearBalance] [SqlElapsed: %v]", time.Since(start))
	}()
	if _, err := do.ExecContext(ctx, strSql, *params.CompanyID, *params.Year, *params.SubjectID); err != nil {
		gLogger.ErrorContext(ctx, "[yearBalance/db/DeleteYearBalance] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
...
func (dao *YearBalanceDao) BatchDeleteYearBalance(ctx context.Context, do DbOperator, filter map[string]interface{}) error {
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/BatchDeleteYearBalance] sql %s with values %v", strSql, values)
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/BatchDeleteYearBalance] [SqlElapsed: %v]", time.Since(start))
	}()
	if _, err := do.ExecContext(ctx, strSql, values...); err != nil {
		gLogger.ErrorContext(ctx, "[yearBalance/db/BatchDeleteYearBalance] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
...
func (dao *YearBalanceDao) UpdateYearBalance(ctx context.Context, do DbOperator,
	...
	start := time.Now()
	gLogger.DebugContext(ctx, "[yearBalance/db/UpdateYearBalance] [sql: %s, values: %v]", strSql, values)
	_, err := do.ExecContext(ctx, strSql, values...)
	gLogger.InfoContext(ctx, "[yearBalance/db/UpdateYearBalance] [SqlElapsed: %v]", time.Since(start))
	if err != nil {
		gLogger.ErrorContext(ctx, "[yearBalance/db/UpdateYearBalance] [do.Exec: %s]", err.Error())
		return err
	}
	return nil
}
...
// 	start := time.Now()
// 	gLogger.DebugContext(ctx, "[yearBalance/db/UpdateBalance] [sql: %s, values: %v]", strSql, values)
// 	_, err := do.ExecContext(ctx, strSql, values...)
// 	gLogger.InfoContext(ctx, "[yearBalance/db/UpdateBalance] [SqlElapsed: %v]", time.Since(start))
// 	if err != nil {
// 		gLogger.ErrorContext(ctx, "[yearBalance/db/UpdateBalance] [do.Exec: %s]", err.Error())
// 		return err
// 	}
// 	return nil
// }
...
func (dao *YearBalanceDao) ListYearBalance(ctx context.Context, do DbOperator, filter map[string]interface{},
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/ListYearBalance] sql %s with values %v", strSql, values)
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/ListYearBalance] [SqlElapsed: %v]", time.Since(start))
	}()
	result, err := do.QueryContext(ctx, strSql, values...)
	if err != nil {
		gLogger.ErrorContext(ctx, "[yearBalance/db/ListYearBalance] [do.Query: %s]", err.Error())
	...
	for result.Next() {
		...
		if err != nil {
			gLogger.ErrorContext(ctx, "[yearBalance/db/List] [ScanSnapshot: %s]", err.Error())
			return yearBalSlice, err
		}
...
func (dao *YearBalanceDao) CountByFilter(ctx context.Context, do DbOperator, filter map[string]interface{}) (int, error) {
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/CountByFilter] sql %s with values %v", strSql, values)
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/CountByFilter] [SqlElapsed: %v]", time.Since(start))
	}()
	switch err := do.QueryRowContext(ctx, strSql, values...).Scan(&c); err {
	case nil:
		return c, nil
	case sql.ErrNoRows:
		gLogger.ErrorContext(ctx, "[yearBalance/db/CountByFilter] [Scan: %s]", err.Error())
		return 0, err
	default:
		gLogger.ErrorContext(ctx, "[yearBalance/db/CountByFilter] [Scan: %s]", err.Error())
		return 0, err
...
func (dao *YearBalanceDao) GetAccSubYearStatus(ctx context.Context, do DbOperator, filter map[string]interface{}) (int, error) {
	...
	gLogger.DebugContext(ctx, "[yearBalance/db/GetAccSubYearStatus] sql %s with values %v", strSql, values)
	var iStatus int = 0
	start := time.Now()
	defer func() {
		gLogger.InfoContext(ctx, "[yearBalance/db/GetAccSubYearStatus] [SqlElapsed: %v]", time.Since(start))
	}()
	...
	case sql.ErrNoRows:
		return 0, nil
	default:
		gLogger.ErrorContext(ctx, "[yearBalance/db/GetAccSubYearStatus] [scanAccSubTask: %s]", err.Error())
		return 0, err
	}
}
