...
func (ah *MenuInfoHandlers) ListMenuInfo(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		gLogger.ErrorContext(r.Context(), "[accSub/ListMenuInfo] [HttpRequestParse: %v]", err)
	...
	// 	gLogger.ErrorContext(r.Context(), "lack base param  companyId or subjectId")
	...
	if ccErr != nil {
		gLogger.WarnContext(r.Context(), "[accSub/ListMenuInfo/ServerHTTP] [MenuService.ListMenuInfo: %s]", ccErr.Detail())
		ah.Response(r.Context(), gLogger, w, ccErr, nil)
		return
	}
